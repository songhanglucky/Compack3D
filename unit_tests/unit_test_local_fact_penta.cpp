...
void resetSystem(Real* l2, Real* l1, Real* d, Real* u1, Real* u2, Real* x, const int N) {
    for(int i = 0; i < N; i++) {
        l2[i] = (i > 1      ) * 0.05;
        l1[i] = (i > 0      ) * 0.50;
        u1[i] = (i < (N - 1)) * 0.50;
...
int main(int argc, char* argv[]) {
    ...
    resetSystem(l2, l1, d, u1, u2, x_thm, N_loc);
    thomasSol(x_thm, l2, l1, d, u1, u2, N_loc);

    resetSystem(l2, l1, d, u1, u2, x_pcr, N_loc);
    localFactPenta(fact_prev_2, fact_prev_1, fact_curr, fact_next_1, fact_next_2, l2, l1, d, u1, u2, N_loc, max_sub_sys_size);
    vanillaLocalSolPentaPCR(x_pcr, fact_prev_2, fact_prev_1, fact_curr, fact_next_1, fact_next_2, N_loc, max_sub_sys_size);
