...
typedef cmpk::DistTriSolDimJ<Real, RealComm, cmpk::MemSpace::Device> TriSol;
...
int main(int argc, char* argv[]) {
   ...
   #error "Unrecognizable MPI library. Supported libraries are OpenMPI and MVAPICH2."
    ...
    // SET LINEAR SYSTEM //
    ///////////////////////
    Real* l = nullptr; cmpk::memAllocArray<cmpk::MemSpace::Host, Real>(&l, NUM_GRIDS_J);
    Real* d = nullptr; cmpk::memAllocArray<cmpk::MemSpace::Host, Real>(&d, NUM_GRIDS_J);
    Real* u = nullptr; cmpk::memAllocArray<cmpk::MemSpace::Host, Real>(&u, NUM_GRIDS_J);
    ...
    cmpk::memAllocArray<cmpk::MemSpace::Device, Real    >(&shared_x_tilde_cur, NUM_GRIDS_I * NUM_GRIDS_K);
    cmpk::memAllocArray<cmpk::MemSpace::Device, Real    >(&shared_x_tilde_nbr, NUM_GRIDS_I * NUM_GRIDS_K);
    cmpk::memAllocArray<cmpk::MemSpace::Device, Real    >(&shared_x_tilde_buf, NUM_GRIDS_I * NUM_GRIDS_K);
    cmpk::memAllocArray<cmpk::MemSpace::Device, RealComm>(&shared_x_comm_prev, NUM_GRIDS_I * NUM_GRIDS_K);
    cmpk::memAllocArray<cmpk::MemSpace::Device, RealComm>(&shared_x_comm_curr, NUM_GRIDS_I * NUM_GRIDS_K);
    cmpk::memAllocArray<cmpk::MemSpace::Device, RealComm>(&shared_x_comm_next, NUM_GRIDS_I * NUM_GRIDS_K);
    ...
    cmpk::memAllocArray<cmpk::MemSpace::Host, Real    >(&shared_x_tilde_send_host, NUM_GRIDS_I * NUM_GRIDS_K);
    cmpk::memAllocArray<cmpk::MemSpace::Host, Real    >(&shared_x_tilde_recv_host, NUM_GRIDS_I * NUM_GRIDS_K);
    cmpk::memAllocArray<cmpk::MemSpace::Host, RealComm>(&shared_x_comm_prev_host , NUM_GRIDS_I * NUM_GRIDS_K);
    cmpk::memAllocArray<cmpk::MemSpace::Host, RealComm>(&shared_x_comm_curr_host , NUM_GRIDS_I * NUM_GRIDS_K);
    cmpk::memAllocArray<cmpk::MemSpace::Host, RealComm>(&shared_x_comm_next_host , NUM_GRIDS_I * NUM_GRIDS_K);
    #endif
        ...
        for (unsigned int j_sub = 0; j_sub < Nj_sub; j_sub++) {
            ...
            for (unsigned int k = 0; k < Nk; k++) {
                x[i * Nj_sub * Nk + j_sub * Nk + k]
                    ...
                    +  COEF_B * (sin(K_WAVE * int(j + 2) * dy) - sin(K_WAVE * int(j - 2) * dy))) / dy;
            } // for k
        } // for j
    } // for i_sub
}
...
RealType checkSoln(RealType* x, const unsigned int Ni, const unsigned int Nj_sub, const unsigned int Nk, MPI_Comm comm) {
    ...
    const RealType dy = DOM_LEN / N_tot;
    const Real K_WAVE_MOD = (2.0 * COEF_A * sin(K_WAVE * dy) + 2.0 * COEF_B * sin(2.0 * K_WAVE * dy)) / dy
                          / (1.0 + 2.0 * COEF_ALP * cos(K_WAVE * dy));
    Real err_loc = 0.0;
    for (unsigned int i = 0; i < Ni; i++) {
        for (unsigned int j_sub = 0; j_sub < Nj_sub; j_sub++) {
            const unsigned int j = j_sub + start_idx; 
