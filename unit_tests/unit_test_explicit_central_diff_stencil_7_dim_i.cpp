...
int main(int argc, char* argv[]) {
    ...
    constexpr Real K_MOD  = 2.0 * COEF_A * sin(K_WAVE * dx) + 2.0 * COEF_B * sin(2.0 * K_WAVE * dx) + 2.0 * COEF_C * sin(3.0 * K_WAVE * dx);
    ...
    memAllocArray<MemSpace::Device, Real>(&f_halo_prev ,           3 * NUM_GRIDS_J * NUM_GRIDS_K);
    memAllocArray<MemSpace::Device, Real>(&f_halo_next ,           3 * NUM_GRIDS_J * NUM_GRIDS_K);
    memAllocArray<MemSpace::Host  , Real>(&f_local_host, NUM_GRIDS_I * NUM_GRIDS_J * NUM_GRIDS_K);
    memAllocArray<MemSpace::Host  , Real>(&f_prev_host ,           3 * NUM_GRIDS_J * NUM_GRIDS_K);
    memAllocArray<MemSpace::Host  , Real>(&f_next_host ,           3 * NUM_GRIDS_J * NUM_GRIDS_K);

    Real* coef_a = nullptr;
    Real* coef_b = nullptr;
    Real* coef_c = nullptr;

    memAllocArray<MemSpace::Device, Real>(&coef_a, NUM_GRIDS_I);
    memAllocArray<MemSpace::Device, Real>(&coef_b, NUM_GRIDS_I);
    memAllocArray<MemSpace::Device, Real>(&coef_c, NUM_GRIDS_I);
    ...
    ////////////////////////////
    {
        ...
        memAllocArray<MemSpace::Host, Real>(&coef_a_host, NUM_GRIDS_I);
        memAllocArray<MemSpace::Host, Real>(&coef_b_host, NUM_GRIDS_I);
        memAllocArray<MemSpace::Host, Real>(&coef_c_host, NUM_GRIDS_I);
        for (unsigned int i = 0; i < NUM_GRIDS_I; i++) {
            coef_a_host[i] = COEF_A;
            coef_b_host[i] = COEF_B;
            coef_c_host[i] = COEF_C;
        }
        deepCopy<MemSpace::Device, MemSpace::Host, Real>(coef_a, coef_a_host, NUM_GRIDS_I);
        deepCopy<MemSpace::Device, MemSpace::Host, Real>(coef_b, coef_b_host, NUM_GRIDS_I);
        deepCopy<MemSpace::Device, MemSpace::Host, Real>(coef_c, coef_c_host, NUM_GRIDS_I);
    ...
    for (int i = 0; i < 3; i++) {
        ...
        const Real x_next = (i + NUM_GRIDS_I) * dx;
        for (unsigned int j = 0; j < NUM_GRIDS_J; j++) {
            const Real y = j * dy;
            for (unsigned int k = 0; k < NUM_GRIDS_K; k++) {
                const Real z = k * dz;
                f_prev_host[i * NUM_GRIDS_J * NUM_GRIDS_K + j * NUM_GRIDS_K + k] = cos(K_WAVE * x_prev) * cos(K_WAVE * y) * cos(K_WAVE * z);
                f_next_host[i * NUM_GRIDS_J * NUM_GRIDS_K + j * NUM_GRIDS_K + k] = cos(K_WAVE * x_next) * cos(K_WAVE * y) * cos(K_WAVE * z);
            }
        }
    }

    deepCopy<MemSpace::Device, MemSpace::Host, Real>(f_local    , f_local_host, NUM_GRIDS_I * NUM_GRIDS_J * NUM_GRIDS_K);
    deepCopy<MemSpace::Device, MemSpace::Host, Real>(f_halo_prev, f_prev_host ,           3 * NUM_GRIDS_J * NUM_GRIDS_K);
    deepCopy<MemSpace::Device, MemSpace::Host, Real>(f_halo_next, f_next_host ,           3 * NUM_GRIDS_J * NUM_GRIDS_K);
    ...
    for (unsigned int i = 0; i < NUM_GRIDS_I; i++) {
        ...
        for (unsigned int j = 0; j < NUM_GRIDS_J; j++) {
            ...
            for (unsigned int k = 0; k < NUM_GRIDS_K; k++) {
                const Real z = k * dz;
                const Real df_ref = -K_MOD * sin(K_WAVE * x) * cos(K_WAVE * y) * cos(K_WAVE * z);
    ...
    printf("Stencil central-diff in dim_i RMS error: %12.5e\n.", err_rms);
    //double err_rms = 0.0;
