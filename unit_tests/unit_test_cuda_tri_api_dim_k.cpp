...
typedef cmpk::DistTriSolDimK<Real, RealComm, cmpk::MemSpace::Device> TriSol;
...
int main(int argc, char* argv[]) {
    ...
    constexpr unsigned int NUM_GRIDS_K = 254;
   ...
   #error "Unrecognizable MPI library. Supported libraries are OpenMPI and MVAPICH2."
    ...
    // SET LINEAR SYSTEM //
    ///////////////////////
    Real* l = nullptr; cmpk::memAllocArray<cmpk::MemSpace::Host, Real>(&l, NUM_GRIDS_K);
    Real* d = nullptr; cmpk::memAllocArray<cmpk::MemSpace::Host, Real>(&d, NUM_GRIDS_K);
    Real* u = nullptr; cmpk::memAllocArray<cmpk::MemSpace::Host, Real>(&u, NUM_GRIDS_K);
    ...
    cmpk::memAllocArray<cmpk::MemSpace::Device, Real    >(&shared_x_tilde_cur, NUM_GRIDS_I * NUM_GRIDS_J);
    cmpk::memAllocArray<cmpk::MemSpace::Device, Real    >(&shared_x_tilde_nbr, NUM_GRIDS_I * NUM_GRIDS_J);
    cmpk::memAllocArray<cmpk::MemSpace::Device, Real    >(&shared_x_tilde_buf, NUM_GRIDS_I * NUM_GRIDS_J);
    cmpk::memAllocArray<cmpk::MemSpace::Device, RealComm>(&shared_x_comm_prev, NUM_GRIDS_I * NUM_GRIDS_J);
    cmpk::memAllocArray<cmpk::MemSpace::Device, RealComm>(&shared_x_comm_curr, NUM_GRIDS_I * NUM_GRIDS_J);
    cmpk::memAllocArray<cmpk::MemSpace::Device, RealComm>(&shared_x_comm_next, NUM_GRIDS_I * NUM_GRIDS_J);
    ...
    cmpk::memAllocArray<cmpk::MemSpace::Host, Real    >(&shared_x_tilde_send_host, NUM_GRIDS_I * NUM_GRIDS_J);
    cmpk::memAllocArray<cmpk::MemSpace::Host, Real    >(&shared_x_tilde_recv_host, NUM_GRIDS_I * NUM_GRIDS_J);
    cmpk::memAllocArray<cmpk::MemSpace::Host, RealComm>(&shared_x_comm_prev_host , NUM_GRIDS_I * NUM_GRIDS_J);
    cmpk::memAllocArray<cmpk::MemSpace::Host, RealComm>(&shared_x_comm_curr_host , NUM_GRIDS_I * NUM_GRIDS_J);
    cmpk::memAllocArray<cmpk::MemSpace::Host, RealComm>(&shared_x_comm_next_host , NUM_GRIDS_I * NUM_GRIDS_J);
    #endif
        ...
        for (unsigned int j = 0; j < Nj; j++) {
            for (unsigned int k_sub = 0; k_sub < Nk_sub; k_sub++) {
                ...
                x[i * Nj * Nk_sub + j * Nk_sub + k_sub]
                    ...
                    +  COEF_B * (sin(K_WAVE * int(k + 2) * dz) - sin(K_WAVE * int(k - 2) * dz))) / dz;
            } // for k
        } // for j
    } // for i_sub
}
...
RealType checkSoln(RealType* x, const unsigned int Ni, const unsigned int Nj, const unsigned int Nk_sub, MPI_Comm comm) {
    ...
    const RealType dz = DOM_LEN / N_tot;
    const Real K_WAVE_MOD = (2.0 * COEF_A * sin(K_WAVE * dz) + 2.0 * COEF_B * sin(2.0 * K_WAVE * dz)) / dz
                          / (1.0 + 2.0 * COEF_ALP * cos(K_WAVE * dz));
    Real err_loc = 0.0;
    for (unsigned int i = 0; i < Ni; i++) {
        for (unsigned int j = 0; j < Nj; j++) {
            for (unsigned int k_sub = 0; k_sub < Nk_sub; k_sub++) {
