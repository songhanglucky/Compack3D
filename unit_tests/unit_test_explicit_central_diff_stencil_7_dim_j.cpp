...
int main(int argc, char* argv[]) {
    ...
    constexpr Real COEF_A =  (3.0 /  4.0) / dy;
    constexpr Real COEF_B = -(3.0 / 20.0) / dy;
    constexpr Real COEF_C =  (1.0 / 60.0) / dy;

    constexpr Real K_WAVE = 2.0;
    constexpr Real K_MOD  = 2.0 * COEF_A * sin(K_WAVE * dy) + 2.0 * COEF_B * sin(2.0 * K_WAVE * dy) + 2.0 * COEF_C * sin(3.0 * K_WAVE * dy);
    ...
    memAllocArray<MemSpace::Device, Real>(&f_halo_prev , NUM_GRIDS_I *           3 * NUM_GRIDS_K);
    memAllocArray<MemSpace::Device, Real>(&f_halo_next , NUM_GRIDS_I *           3 * NUM_GRIDS_K);
    memAllocArray<MemSpace::Host  , Real>(&f_local_host, NUM_GRIDS_I * NUM_GRIDS_J * NUM_GRIDS_K);
    memAllocArray<MemSpace::Host  , Real>(&f_prev_host , NUM_GRIDS_I *           3 * NUM_GRIDS_K);
    memAllocArray<MemSpace::Host  , Real>(&f_next_host , NUM_GRIDS_I *           3 * NUM_GRIDS_K);

    Real* coef_a = nullptr;
    Real* coef_b = nullptr;
    Real* coef_c = nullptr;

    memAllocArray<MemSpace::Device, Real>(&coef_a, NUM_GRIDS_J);
    memAllocArray<MemSpace::Device, Real>(&coef_b, NUM_GRIDS_J);
    memAllocArray<MemSpace::Device, Real>(&coef_c, NUM_GRIDS_J);
    ...
    ////////////////////////////
    {
        ...
        memAllocArray<MemSpace::Host, Real>(&coef_a_host, NUM_GRIDS_J);
        memAllocArray<MemSpace::Host, Real>(&coef_b_host, NUM_GRIDS_J);
        memAllocArray<MemSpace::Host, Real>(&coef_c_host, NUM_GRIDS_J);
        for (unsigned int j = 0; j < NUM_GRIDS_J; j++) {
            coef_a_host[j] = COEF_A;
            coef_b_host[j] = COEF_B;
            coef_c_host[j] = COEF_C;
        }
        deepCopy<MemSpace::Device, MemSpace::Host, Real>(coef_a, coef_a_host, NUM_GRIDS_J);
        deepCopy<MemSpace::Device, MemSpace::Host, Real>(coef_b, coef_b_host, NUM_GRIDS_J);
        deepCopy<MemSpace::Device, MemSpace::Host, Real>(coef_c, coef_c_host, NUM_GRIDS_J);
    ...
    for (unsigned int i = 0; i < NUM_GRIDS_I; i++) {
        const Real x = i * dx;
        for (int j = 0; j < 3; j++) {
            const Real y_prev = (j -           3) * dy;
            const Real y_next = (j + NUM_GRIDS_J) * dy;
            for (unsigned int k = 0; k < NUM_GRIDS_K; k++) {
                const Real z = k * dz;
                f_prev_host[i * 3 * NUM_GRIDS_K + j * NUM_GRIDS_K + k] = cos(K_WAVE * x) * cos(K_WAVE * y_prev) * cos(K_WAVE * z);
                f_next_host[i * 3 * NUM_GRIDS_K + j * NUM_GRIDS_K + k] = cos(K_WAVE * x) * cos(K_WAVE * y_next) * cos(K_WAVE * z);
            }
        }
    }
    ...
    deepCopy<MemSpace::Device, MemSpace::Host, Real>(f_halo_prev, f_prev_host , NUM_GRIDS_I *           3 * NUM_GRIDS_K);
    deepCopy<MemSpace::Device, MemSpace::Host, Real>(f_halo_next, f_next_host , NUM_GRIDS_I *           3 * NUM_GRIDS_K);
    ...
    for (unsigned int i = 0; i < NUM_GRIDS_I; i++) {
        ...
        for (unsigned int j = 0; j < NUM_GRIDS_J; j++) {
            ...
            for (unsigned int k = 0; k < NUM_GRIDS_K; k++) {
                const Real z = k * dz;
                const Real df_ref = -K_MOD * cos(K_WAVE * x) * sin(K_WAVE * y) * cos(K_WAVE * z);
    ...
    printf("Stencil central-diff in dim_j RMS error: %12.5e\n.", err_rms);
    //double err_rms = 0.0;
