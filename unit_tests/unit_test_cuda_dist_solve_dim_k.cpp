...
#error "Unrecognizable MPI library. Supported libraries are OpenMPI and MVAPICH2."
    ...
    constexpr unsigned int N_sub = Nk;
    ...
    memAllocArray<MemSpace::Device, Real>(&x_tilde_cur, Ni * Nj *  2);
    memAllocArray<MemSpace::Device, Real>(&x_tilde_nbr, Ni * Nj *  2);
    memAllocArray<MemSpace::Device, Real>(&x_tilde_buf, Ni * Nj *  2);
    ...
    memAllocArray<MemSpace::Device, RealComm>(&x_comm_prev, Ni * Nj *  2);
    memAllocArray<MemSpace::Device, RealComm>(&x_comm_curr, Ni * Nj *  2);
    memAllocArray<MemSpace::Device, RealComm>(&x_comm_next, Ni * Nj *  2);
    cudaCheckError();
    ...
    memAllocArray<MemSpace::Host, Real>(&x_tilde_host, Ni * Nk * 2); // DEBUG ONLY

    ////////////////////////////////////
    //---- START SOLUTION PROCESS ----//

    Real* x_loc = &x[2];
    localSolPentaPCRDimK<Real>(x_loc, fact_local_prev_2, fact_local_prev_1, fact_local_curr, fact_local_next_1, fact_local_next_2, max_local_fact_size, Ni, Nj, Nk-2, arr_stride_i, arr_stride_j);
    copySlicesFrom3DArrayDimK<Real>(x_tilde_buf, &x_loc[N_local-2], Ni, Nj, 2, arr_stride_i, arr_stride_j); // get y_prev_bot
    deviceFence();
    MPI_Irecv(x_tilde_nbr, 2*Ni*Nj, MPIDataType<Real>::value, (rank - 1 + Np) % Np, 1, MPI_COMM_WORLD, &mpi_reqs[0]);
    MPI_Isend(x_tilde_buf, 2*Ni*Nj, MPIDataType<Real>::value, (rank + 1     ) % Np, 1, MPI_COMM_WORLD, &mpi_reqs[1]);
    copySlicesFrom3DArrayDimK<Real>(x_tilde_cur, x, Ni, Nj, 2, arr_stride_i, arr_stride_j);
    MPI_Waitall(2, mpi_reqs, mpi_stats);
    copySlicesFrom3DArrayDimK<Real>(x_tilde_buf, x_loc, Ni, Ni, 2, arr_stride_i, arr_stride_j); // get y_curr_head
    calcReducedSystemRHSLocal(x_tilde_cur, x_tilde_nbr, x_tilde_buf, Li_tilde_tail, Ui_tilde_head, Ni*Nj);
    ...
    deviceFence();
    MPI_Irecv(x_tilde_nbr, 2*Ni*Nj, MPIDataType<Real>::value, (rank + 1     ) % Np, 1, MPI_COMM_WORLD, &mpi_reqs[0]);
    MPI_Isend(x_tilde_cur, 2*Ni*Nj, MPIDataType<Real>::value, (rank - 1 + Np) % Np, 1, MPI_COMM_WORLD, &mpi_reqs[1]);
    copySlicesTo3DArrayDimK<Real>(x, x_tilde_cur, Ni, Nj, 2, arr_stride_i, arr_stride_j);
    MPI_Waitall(2, mpi_reqs, mpi_stats);
    updateLocalSolDimK<Real>(x_loc, x_tilde_cur, x_tilde_nbr, Si, Ri, Ni, Nj, Nk-2, Nj*Nk, Nk);
    deviceFence();
    cudaCheckError();
