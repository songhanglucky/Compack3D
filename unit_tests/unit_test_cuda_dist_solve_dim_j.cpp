...
#error "Unrecognizable MPI library. Supported libraries are OpenMPI and MVAPICH2."
    ...
    constexpr unsigned int N_sub = Nj;
    ...
    memAllocArray<MemSpace::Device, Real>(&x_tilde_cur, Ni *  2 * Nk);
    memAllocArray<MemSpace::Device, Real>(&x_tilde_nbr, Ni *  2 * Nk);
    memAllocArray<MemSpace::Device, Real>(&x_tilde_buf, Ni *  2 * Nk);
    ...
    memAllocArray<MemSpace::Device, RealComm>(&x_comm_prev, Ni *  2 * Nk);
    memAllocArray<MemSpace::Device, RealComm>(&x_comm_curr, Ni *  2 * Nk);
    memAllocArray<MemSpace::Device, RealComm>(&x_comm_next, Ni *  2 * Nk);
    cudaCheckError();
    ...
    memAllocArray<MemSpace::Host, Real>(&x_tilde_host, Ni * 2 * Nk); // DEBUG ONLY

    ////////////////////////////////////
    //---- START SOLUTION PROCESS ----//

    Real* x_loc = &x[2 * arr_stride_j];
    localSolPentaPCRDimJ<Real>(x_loc, fact_local_prev_2, fact_local_prev_1, fact_local_curr, fact_local_next_1, fact_local_next_2, max_local_fact_size, Ni, Nj-2, Nk, arr_stride_i, arr_stride_j);
    copySlicesFrom3DArrayDimJ<Real>(x_tilde_buf, &x_loc[(N_local-2) * arr_stride_j], Ni, 2, Nk, arr_stride_i, arr_stride_j); // get y_prev_bot
    deviceFence();
    MPI_Irecv(x_tilde_nbr, 2*Ni*Nk, MPIDataType<Real>::value, (rank - 1 + Np) % Np, 1, MPI_COMM_WORLD, &mpi_reqs[0]);
    MPI_Isend(x_tilde_buf, 2*Ni*Nk, MPIDataType<Real>::value, (rank + 1     ) % Np, 1, MPI_COMM_WORLD, &mpi_reqs[1]);
    copySlicesFrom3DArrayDimJ<Real>(x_tilde_cur, x, Ni, 2, Nk, arr_stride_i, arr_stride_j);
    MPI_Waitall(2, mpi_reqs, mpi_stats);
    copySlicesFrom3DArrayDimJ<Real>(x_tilde_buf, x_loc, Ni, 2, Nk, arr_stride_i, arr_stride_j); // get y_curr_head
    calcReducedSystemRHSLocal(x_tilde_cur, x_tilde_nbr, x_tilde_buf, Li_tilde_tail, Ui_tilde_head, Ni*Nk);
    distSolve<Real, RealComm, mem_space>(x_tilde_cur, x_comm_prev, x_comm_curr, x_comm_next, fact_dist_prev, fact_dist_curr, fact_dist_next, Ni*Nk, Np, MPI_COMM_WORLD, nullptr, nullptr, nullptr);
    deviceFence();
    MPI_Irecv(x_tilde_nbr, 2*Ni*Nk, MPIDataType<Real>::value, (rank + 1     ) % Np, 1, MPI_COMM_WORLD, &mpi_reqs[0]);
    MPI_Isend(x_tilde_cur, 2*Ni*Nk, MPIDataType<Real>::value, (rank - 1 + Np) % Np, 1, MPI_COMM_WORLD, &mpi_reqs[1]);
    copySlicesTo3DArrayDimJ<Real>(x, x_tilde_cur, Ni, 2, Nk, arr_stride_i, arr_stride_j);
    MPI_Waitall(2, mpi_reqs, mpi_stats);
    updateLocalSolDimJ<Real>(x_loc, x_tilde_cur, x_tilde_nbr, Si, Ri, Ni, Nj-2, Nk, Nj*Nk, Nk);
    deviceFence();
    cudaCheckError();
...
template<typename RealType>
void setRHS(RealType* x, const unsigned int Ni, const unsigned int Nj_sub, const unsigned int Nk, MPI_Comm comm) {
    int rank, Np;
    MPI_Comm_rank(comm, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &Np);
    const unsigned int start_idx = rank * Nj_sub;
    const unsigned int N_tot     = Np   * Nj_sub;
    const RealType dy = DOM_LEN / N_tot;
    for (unsigned int i = 0; i < Ni; i++) {
        for (unsigned int j_sub = 0; j_sub < Nj_sub; j_sub++) {
            const unsigned int j = j_sub + start_idx; 
            for (unsigned int k = 0; k < Nk; k++) {
                x[i * Nj_sub * Nk + j_sub * Nk + k]
                    = (COEF_A * (sin(K_WAVE * int(j + 1) * dy) - sin(K_WAVE * int(j - 1) * dy))
                    +  COEF_B * (sin(K_WAVE * int(j + 2) * dy) - sin(K_WAVE * int(j - 2) * dy))
                    +  COEF_C * (sin(K_WAVE * int(j + 3) * dy) - sin(K_WAVE * int(j - 3) * dy))) / dy;
            } // for k
        } // for j
    } // for i_sub
}
...
template<typename RealType>
RealType checkSoln(RealType* x, const unsigned int Ni, const unsigned int Nj_sub, const unsigned int Nk, MPI_Comm comm) {
    int rank, Np;
    MPI_Comm_rank(comm, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &Np);
    const unsigned int start_idx = rank * Nj_sub;
    ...
    const RealType dy = DOM_LEN / N_tot;
    const Real K_WAVE_MOD = (2.0 * COEF_A * sin(K_WAVE * dy) + 2.0 * COEF_B * sin(2.0 * K_WAVE * dy) + 2.0 * COEF_C * sin(3.0 * K_WAVE * dy)) / dy
                          / (1.0 + 2.0 * COEF_ALP * cos(K_WAVE * dy) + 2.0 * COEF_BET * cos(2.0 * K_WAVE * dy));
    Real err_loc = 0.0;
    for (unsigned int i = 0; i < Ni; i++) {
        for (unsigned int j_sub = 0; j_sub < Nj_sub; j_sub++) {
            const unsigned int j = j_sub + start_idx; 
            const Real x_ref = K_WAVE_MOD * cos(K_WAVE * int(j) * dy);
            for (unsigned int k = 0; k < Nk; k++) {
                Real err_loc_j = x_ref - x[i * Nj_sub * Nk + j_sub * Nk + k];
                err_loc += err_loc_j * err_loc_j;
            } // k
        } // j
    } // i
    Real err;
    MPI_Allreduce(&err_loc, &err, 1, MPIDataType<Real>::value, MPI_SUM, comm);
    err = sqrt((err / N_tot) / (Ni * Nk));
    ...
    if (err > 1e-13) {
        for (int r = 0; r < Np; r++) {
            if (r == rank) {
                ...
                for (unsigned int j_sub = 0; j_sub < Nj_sub; j_sub ++) {
                    const Real x_ref = K_WAVE_MOD * cos(K_WAVE * (j_sub + start_idx) * dy);
                    const Real x_num = x[i * Nj_sub * Nk + j_sub * Nk + k];
                    printf("%3d, %12.5e, %12.5e, %12.5e\n", j_sub + start_idx, x_ref, x_num, x_ref - x_num);
                }
            }
            MPI_Barrier(comm);
        } // for r
